...
### Improved
- **Restore test**: show percentage of total snapshot during restore progress (e.g., `progress: 1.2G extracted (17% of total)`) — uses `restic stats` to get total snapshot size before starting
...
### Fixed
- **Gateway manager**: fix update not reinstalling scripts when git repo is already pulled but installed version is outdated — now detects version mismatch and forces reinstall
...
### Fixed
- **Restore test**: reduce SSH key copy timeout from 15s to 10s (ConnectTimeout 5s) for faster fallback to manual instructions
